 ...
 * Author: angel
 *
 * Created on 20 de agosto de 2021, 08:44 PM
 */
...
// funcion encargada de seleccionar el pre_escaler el TMR1 ademas de seleccionar
//que esta funcionara con el FOSC/4 y con el oscilador interno. 
void conf_TMR1(uint8_t pre_escaler) {
    ...
    switch (pre_escaler) {
        case (1):
            T1CONbits.T1CKPS = 0b00;     // para pre_escaler = 1:1
            break;
        case (2):
            T1CONbits.T1CKPS = 0b01;     // para pre_escaler = 1:2
            break;
        case (4):
            T1CONbits.T1CKPS = 0b10;     // para pre_escaler = 1:4
            break;
        case (8):
            T1CONbits.T1CKPS = 0b11;     // para pre_escaler = 1:8
            break;
    }
    return; 
}
...
char centenas(uint16_t cantidad) {
